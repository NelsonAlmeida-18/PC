...
class Account{
    ...
    public Account(int id, float funds){
        this.id=id;
        this.funds=funds;
    }
...
class myMultiThread extends Thread{
    ...
    public synchronized void transfer(int srcAccount, int destAccount, float value){
        lock.lock();
        this.banco.withdraw(srcAccount, value);
        this.banco.deposit(destAccount, value);
        System.out.println("Acabou transfer thread "+threadNum+"Total conta src "+this.totalBalance(srcAccount));
        lock.unlock();
    }
