...
class Bank {
  ...
  public void transfer(int srcId, int targetId, float val) throws InvalidAmmount, NotEnoughFunds, InvalidAccount {

    // Não teria de levar locks para previnir que alguma conta seja eliminada
    // enquanto esta
    // transação está a ser executada?
    l.lock();
    Account src = this.contas.get(srcId);
    Account dest = this.contas.get(targetId);
    l.unlock();
