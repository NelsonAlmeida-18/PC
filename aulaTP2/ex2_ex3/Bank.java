...
class Bank {
  ...
  public synchronized void Deposit(int id, float val) throws InvalidDepositValue, InvalidAccount {
  ...
  public synchronized void Withdraw(int id, float val) throws NotEnoughFunds, InvalidAccount {
    accs[id].withdraw(val);
    this.totalBalance -= val;
  }

  public synchronized void Transfer(int srcId, int destId, float val) throws InvalidAccount, NotEnoughFunds, InvalidDepositValue{
...
class Account {
  ...
  public void deposit(float val) throws InvalidDepositValue {
    if (val > 0)
      this.balance += val;
    else
  ...
  public void withdraw(float val) throws NotEnoughFunds {
    if (val <= balance) {
      this.balance -= val;
    } else
        ...
        new Thread(() -> {
          ...
          try{
            if (operation == 0) {
              ...
              b.Withdraw(randomGenerator.nextInt(numAccounts), randomGenerator.nextInt(100)+randomGenerator.nextFloat());
            }
            else if(operation==2){
              b.Transfer(randomGenerator.nextInt(numAccounts), randomGenerator.nextInt(numAccounts), randomGenerator.nextInt(100)+randomGenerator.nextFloat());
            }
          }
          catch(Exception e){
            e.printStackTrace();
